...
我们还知道字符串 $w$ 的前几个后缀（按长度降序考虑）全部包含于这个等价类，且所有其它后缀（至少有一个——空后缀）在其它的等价类中。我们记 $t$ 为最长的这样的后缀，然后将 $w$ 的后缀链接连到 $t$ 上。

换句话说，$v$ 的 **后缀链接** $\operatorname{link}(v)$ 指向对应于 $w$ 最长后缀的另一个 $\operatorname{endpos}$ 等价类的状态。
...
- 现在我们按以下流程进行（从状态 $\textit{last}$ 开始）。如果还不存在经过字符 $c$ 的转移，我们就添加一个到状态 $\textit{cur}$ 的转移，遍历后缀链接。如果在某个点已经存在经过字符 $c$ 的转移，我们就停下来，并将这个状态标记为 $p$。
